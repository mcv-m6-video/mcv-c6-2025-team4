...
gt_dict = {}
for item in gt_data:
    # For each frame, store a list of boxes
    frame_no = item["frame"]
...
for idx, frame_rgb in enumerate(test_frames, start=training_end):
    ...
    background_mask = improved_classify_frame(frame_rgb, bg_mean, bg_variance)
    ...
    # Get ground truth boxes for the current frame (if available)
    ...
    if gt_boxes:
        ...
        print(
            f"Frame {idx}: Precision={precision:.2f}, Recall={recall:.2f}, Avg IoU={avg_iou:.2f}, TP={tp}, FP={fp}, FN={fn}")
    else:
        print(f"Frame {idx}: No GT available.")

    # For pixel-level ROC evaluation:
    if gt_boxes:
        gt_mask = metrics.generate_gt_mask(frame_rgb.shape, gt_boxes)
        # Compute pixel-level metrics comparing the predicted foreground mask with GT mask
        TPR, FPR = metrics.compute_pixel_metrics(background_mask, gt_mask)
        print(f"Frame {idx}: Pixel-level TPR={TPR:.2f}, FPR={FPR:.2f}")
