...
class ActionSpotDataset(Dataset):
    ...
    def _load_clips(self):
        ...
        for game in self._games:
            ...
            self.videos.append((video, num_frames, FPS_SN / self._stride))
            self._video_lengths[video] = num_frames
    ...
    def _get_one(self, idx):
        ...
        # Extract video name
        video = os.path.basename(frames_path[0])  # base folder = video name
        start = frames_path[1] // self._stride
    ...
    def __getitem__(self, idx):
        ret = self._get_one(idx)

        video = os.path.basename(self._frame_paths[idx][0])
        start = self._frame_paths[idx][1] // self._stride
