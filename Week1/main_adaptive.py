...
def improved_classify_frame(frame, background_mean, background_variance, threshold_factor=8, min_area=500):
    ...
    - threshold_factor: float, optional
        The threshold factor for classification (default is 8).
...
# Create an instance of the adaptive Gaussian model
...
# Load ground truth annotations from the XML file
...
# Process the remaining 75% of frames for testing
...
for idx, frame_rgb in enumerate(test_frames, start=training_end):
    ...
    # Process the frame: obtain the background mask and update the adaptive model
    ...
    # Convert mask to an 8-bit format for visualization
    mask_8bit = (background_mask).astype(np.uint8)
    mask_colored = cv2.cvtColor(mask_8bit, cv2.COLOR_GRAY2BGR)

    # Extract bounding boxes from the foreground mask (assuming extract_bounding_boxes is defined in metrics)
